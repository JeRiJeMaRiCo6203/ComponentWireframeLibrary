...
//     // console.log('semua', wireframesQuery);

//     // const allCategoriesOfWireframes = wireframesQuery.flatMap(wireframes => wireframes.categories);
//     // console.log('yang dimiliki', allCategoriesOfWireframes);

//     // const missingCategories = allCategoriesOfWireframes.filter((category) => !categoriesArray.includes(category));
//     // console.log('yang hilang', missingCategories);
...
//       const missingCategories = allCategories.filter((category) => !categoriesArray.includes(category.name));
//       console.log('yang hilang', missingCategories);
...
//       // Step 2: Perform Search with PRIORITY on category match first, then wireframes title
//       if (keywordsArray.length > 0) {
//         searchResults = await prisma.$queryRaw(
//           Prisma.sql`
//             SELECT 
...
//                 WHEN c.name ILIKE ANY(ARRAY[${Prisma.join(
//                   keywordsArray.map((k) => `%${k}%`)
//                 )}]) THEN 1
//                 -- Priority 2: wireframe title match
//                 WHEN w.title ILIKE ANY(ARRAY[${Prisma.join(
//                   keywordsArray.map((k) => `%${k}%`)
//                 )}]) THEN 2
//                 ELSE 3
...
//         // Step 3: Fetch all categories for each wireframe in search results
//         const wireframeIds = searchResults.map((result) => result.id);
  
//         const allCategories = await prisma.$queryRaw(
//           Prisma.sql`
//             SELECT w.id, array_agg(DISTINCT c.name) AS allCategories
//             FROM wireframes w
//             JOIN wireframe_category_editable_relationship wc ON w.id = wc.wireframe_id
//             JOIN categories c ON wc.category_id = c.id
...
//         // Step 4: Combine the full category list back into the search results
//         searchResults = searchResults.map((result) => {
//           const matchedCategories = allCategories.find(
//             (catResult) => catResult.id === result.id
//           );
//           result.categories = matchedCategories ? matchedCategories.allCategories : result.categories;
//           return result;
//         });
//       }
