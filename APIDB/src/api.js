...
app.get("/wireframesByEditables", async (request, response) => {
  try {
    ...
    const editablesArray = editables
      .split(",")
      .map((editable) => editable.trim());
    ...
    const filteredWireframesQuery = await Promise.all(
      wireframesQuery.map(async (wireframe) => {
        ...
        wireframe.editables = remainingEditables.map(
...
app.get("/searchCategories/", async (request, response) => {
  try {
    ...
    const wireframesWithAllDetails = wireframesQuery.map((wireframe) => {
      const details = allCategoriesAndEditablesQuery.find(
        (w) => w.id === wireframe.id
      );
      return {
        ...wireframe,
    ...
    response.json({
      ...
      results: wireframesWithAllDetails,
    });
  } catch (error) {
    console.error("Error fetching wireframes:", error);
    response.status(500).send({
      error: "An error occurred while searching wireframes by category.",
    });
  }
});
...
app.post("/insert/", async (request, response) => {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (prisma) => {
      ...
      for (const category of newCategories) {
        for (const editable of newEditables) {
          const relationship =
            await prisma.wireframe_category_editable_relationship.create({
              data: {
    ...
    response.send({
      ...
      wireframeCategoryEditableRelationships:
        result.wireframeCategoryEditableRelationships,
      message:
